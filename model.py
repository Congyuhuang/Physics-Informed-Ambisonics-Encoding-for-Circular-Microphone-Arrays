...
class Model(Net):
    def __init__(self, in_channels, out_channels, atte_dim, emb_dim, emb_ks, emb_hs, hidden_dim, n_head, n_layers,
        ...
        self.channel_conv = nn.Sequential(
            nn.Conv2d(emb_dim, emb_dim * self.out_channels, (3, 3), padding=(1, 1)),
            nn.Mish(inplace=True),
            nn.GroupNorm(self.out_channels, emb_dim * self.out_channels)
        )

        self.down_conv = nn.Conv2d(emb_dim, 2, (3, 3), padding=(1, 1))
        self.modulator = Dir_Feature()
        #self.down = LinearGroup(in_features=emb_dim, out_features=2, num_groups=self.out_channels)
        #self.linear = nn.Linear(in_features=emb_dim, out_features=self.out_channels*2)
    ...
    def forward(self, mic):
        ...
        #out = self.linear(batch.transpose(3, 1)).transpose(3, 1).reshape(B, self.out_channels, 2, T, F)
        # batch = self.channel_conv(batch).reshape(B, self.out_channels, self.emb_dim, T, F).reshape(-1, self.emb_dim, T,
        #                                                                                             F)   #[B*Channel, C, T, F]
        batch = self.channel_conv(batch)
        batch = self._modulate(mic, batch)
        batch = batch.reshape(B, self.out_channels, self.emb_dim, T, F).reshape(-1, self.emb_dim, T, F)

        # batch = self.channel_conv(batch).reshape(B, self.out_channels, self.emb_dim, T, F).permute(0, 3, 4, 1, 2)   #[B, T, F, Out, Emb_dim] ->down -> [B, T, F, Out, 2]
        # out = self.down(batch).permute(0, 3, 4, 1, 2)
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 16, 188, 257)).cuda()
    y = model(x)
