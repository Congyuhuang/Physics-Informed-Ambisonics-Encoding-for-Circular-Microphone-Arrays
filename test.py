    ...
    for data in data_pbar:
        ...
        with torch.no_grad():
            ...
            for batch in range(foa_audio.shape[0]):
                ...
                _, _LQ, _LA = calculate_ambiqual(_foa_audio.detach().cpu().numpy().T, _pred_t.detach().cpu().numpy().T, sample_rate=24000, n_channels=4 , intensity_threshold=-180, elc=0, ignore_freq_bands=0)
                LA += _LA
                LQ += _LQ
